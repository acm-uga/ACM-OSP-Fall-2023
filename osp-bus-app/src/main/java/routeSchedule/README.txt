...
DailySchedule:
HashMap<DayOfWeek, OperatingTime[]>
 
OperatingDate:
Start and End LocalDate's

OperatingTime:
Start and End LocalTime's
...
ENCODING GUIDELINES:---------------------------------------
At least one operating window must exist.
0-unlimited operating dates can exist (0 indicates indefinite operation with the DailySchedule that follows).
Multiple operating dates must be ordered chronologically, by the start date if a range.
At least one day of the week must be defined in a daily schedule UNLESS only a single parent date is specified, in which
only times are necessary (no ?: needed).
Multiple day indicators must be ordered chronologically in sets AND by the first day indicator in the daily schedule.
Regardless of position in sequence, every list of times must end in ; when a day or days is/are specified.
0-unlimited operating times can exist per day.
Multiple operating times must be ordered chronologically by the start time.
 
ENCODING SYMBOLS:------------------------------------------
, relates multiple parents to child data and/or one (or more) parent(s) to multiple children
- indicates a range or pre/post continuity (always inclusive)
: indicates the following operating times until ; belong to the preceding day indicators
{ indicates the following daily schedule until } belongs to the preceding operating dates
| indicates a new encoded Operating Window
...
A lack of day of week indicators (UMTWRFS) indicates the contained operating times apply to every day included in the parent operating date.
...
From the date of instantiation until 12/12/23...
...
From the date of instantiation until 1/23/24, from 1/25/24 to 2/27/24, on 3/1/24, and from 3/3/24 onwards...
The route operates continuously from Sunday at 5:25pm to Monday at 2:30am EST;
